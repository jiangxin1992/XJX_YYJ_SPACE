...
-(void)Notifications
{
    ...
    //        新的线下活动通知
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(NEWLIVESERIES_NOT_Action) name:@"NEWLIVESERIES_NOT" object:nil];
    //        已报名线下活动开始的通知
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(STARTLIVESERIES_Action) name:@"STARTLIVESERIES_NOT" object:nil];
...
//        新的线下活动通知
-(void)NEWLIVESERIES_NOT_Action
{
    [self updateBtnStatus:2];
    [_ddayCtn PushDDAYDetailView];
}
//        已报名的线下活动通知
-(void)STARTLIVESERIES_Action
{
    [self updateBtnStatus:2];
    [_ddayCtn PushDDAYDetailView];
}
